...
const TILE_COLORS: Record<TileType, number> = {
  ...
  [TileType.Plant]: 0xede7db, // chão por baixo; a planta é desenhada como círculo
...
export class Tilemap {
  ...
  isSolidAt(tileX: number, tileY: number): boolean {
    if (tileX < 0 || tileY < 0 || tileX >= this.map.cols || tileY >= this.map.rows) return true;
    return isSolid(this.map.tiles[tileY][tileX]);
  }
